...
if [ $? -ne 0 ]; then
    ...
    pip install fastapi uvicorn openai python-dotenv tencentcloud-sdk-python-hunyuan
    if [ $? -ne 0 ]; then
...
# 检查是否有.env文件
if [ -f ".env" ]; then
    print_info "检测到.env文件，将使用其中的配置"
else
    # 提示用户输入环境变量
    print_info "未检测到.env文件，将创建新的配置"
    echo ""
    
    read -p "请输入腾讯云Secret ID [如不输入将使用测试值]: " TENCENT_SECRET_ID
    TENCENT_SECRET_ID=${TENCENT_SECRET_ID:-your_secret_id}
    
    read -p "请输入腾讯云Secret Key [如不输入将使用测试值]: " TENCENT_SECRET_KEY
    TENCENT_SECRET_KEY=${TENCENT_SECRET_KEY:-your_secret_key}
...
# 导出环境变量
export TENCENT_SECRET_ID=$(grep TENCENT_SECRET_ID .env | cut -d '=' -f2)
export TENCENT_SECRET_KEY=$(grep TENCENT_SECRET_KEY .env | cut -d '=' -f2)
